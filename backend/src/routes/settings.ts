...
import { createApiKeySchema, createWebhookSchema, updateWebhookSchema, updateSettingSchema } from '../schemas/settings.schema.js'
...
settingsRouter.patch('/:key', zValidator('json', updateSettingSchema), async (c) => {
  const user = c.get('user')
