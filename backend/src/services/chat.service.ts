...
export async function listSessions(workspaceId: string, input: ListSessionsInput) {
...
export async function createSession(workspaceId: string, userId: string, input: CreateSessionInput) {
  const [session] = await db
    .insert(chatSessions)
    .values({
      workspaceId,
...
function formatSession(session: typeof chatSessions.$inferSelect) {
  return {
    id: session.id,
...
function formatMessage(msg: typeof chatMessages.$inferSelect) {
  return {
    id: msg.id,
