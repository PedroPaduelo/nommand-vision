...
export async function list(workspaceId: string, filters: ListDeploysInput) {
...
export async function create(workspaceId: string, userId: string, input: CreateDeployInput) {
...
export async function streamLogs(id: string) {
  await get(id)
...
export async function listEnvVars(deployId: string) {
  const vars = await db
    .select()
    .from(deployEnvVars)
...
export async function setEnvVar(deployId: string, input: SetEnvVarInput) {
...
function formatDeploy(deploy: typeof deploys.$inferSelect) {
  return {
    id: deploy.id,
