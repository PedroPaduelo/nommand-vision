...
export async function getOverview(workspaceId: string) {
  ...
  let globalStatus: 'operational' | 'degraded' | 'major_outage' = 'operational'
  if (hasMajor) globalStatus = 'major_outage'
  else if (!allOperational) globalStatus = 'degraded'
...
export async function createService(workspaceId: string, input: CreateServiceInput) {
  const [service] = await db
    .insert(services)
    .values({
      workspaceId,
...
export async function updateServiceStatus(id: string, status: string, responseTimeMs?: number) {
  const [service] = await db
    .update(services)
    .set({
...
export async function getUptimePercentage(serviceId: string, days: number = 30) {
  const since = new Date()
  since.setDate(since.getDate() - days)
  ...
  const [result] = await db
    .select({
      ...
      up: sql<number>`count(*) filter (where ${uptimeChecks.status} = 'up')`,
    })
    .from(uptimeChecks)
    .where(and(eq(uptimeChecks.serviceId, serviceId), gte(uptimeChecks.checkedAt, since)))

  const total = Number(result.total)
  if (total === 0) return { percentage: 100, total: 0, up: 0 }
...
export async function listIncidents(workspaceId: string, filters: ListIncidentsInput) {
...
export async function recordCheck(serviceId: string, status: string, responseTimeMs: number) {
  ...
  await db
    .update(services)
    .set({
      status: status === 'up' ? 'operational' : status === 'degraded' ? 'degraded' : 'major_outage' as any,
      responseTimeMs,
      lastCheckAt: new Date(),
      updatedAt: new Date(),
    })
...
function formatService(service: typeof services.$inferSelect) {
  return {
    id: service.id,
...
function formatIncident(incident: typeof incidents.$inferSelect) {
  return {
    id: incident.id,
