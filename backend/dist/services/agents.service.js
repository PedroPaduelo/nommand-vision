...
import { agents, agentRuns, projects } from '../db/schema/index.js';
...
export async function list(workspaceId, filters) {
    ...
    if (filters.search) {
        ...
        conditions.push(or(like(agents.name, searchPattern), sql `COALESCE(${agents.description}, '') LIKE ${searchPattern}`));
    }
...
export async function listRuns(agentId, filters) {
...
export async function listByProject(workspaceId, projectId) {
