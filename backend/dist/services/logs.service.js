...
export async function list(workspaceId, input) {
    const conditions = [eq(logs.workspaceId, workspaceId)];
    if (input.level) {
...
export async function createBatch(workspaceId, items) {
...
export async function search(workspaceId, input) {
    const offset = (input.page - 1) * input.perPage;
    const rows = await db
        .select()
        .from(logs)
        .where(and(eq(logs.workspaceId, workspaceId), ilike(logs.message, `%${input.q}%`)))
        .orderBy(desc(logs.createdAt))
        .limit(input.perPage)
        .offset(offset);
    const [{ count }] = await db
...
export async function deleteOld(workspaceId, olderThanDays) {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - olderThanDays);
    const result = await db
        .delete(logs)
        .where(and(eq(logs.workspaceId, workspaceId), lte(logs.createdAt, cutoff)))
