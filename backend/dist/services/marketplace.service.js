...
export async function getItem(id, workspaceId) {
    const [item] = await db
        .select()
        .from(marketplaceItems)
    ...
    const [install] = await db
        .select()
        .from(marketplaceInstalls)
        .where(and(eq(marketplaceInstalls.workspaceId, workspaceId), eq(marketplaceInstalls.itemId, id)))
        .limit(1);
    const [{ installCount }] = await db
        .select({ installCount: sql `count(*)` })
        .from(marketplaceInstalls)
...
export async function installItem(workspaceId, itemId, userId, input) {
    const [item] = await db
        .select()
        .from(marketplaceItems)
...
export async function uninstallItem(workspaceId, itemId) {
    const [install] = await db
...
export async function listInstalled(workspaceId) {
    const data = await db
        ...
        .from(marketplaceInstalls)
        .where(eq(marketplaceInstalls.workspaceId, workspaceId));
    const items = await Promise.all(data.map(async (install) => {
        const [item] = await db
            .select()
            .from(marketplaceItems)
...
export async function rateItem(workspaceId, itemId, input) {
    const [item] = await db
        .select()
        .from(marketplaceItems)
        .where(eq(marketplaceItems.id, itemId))
